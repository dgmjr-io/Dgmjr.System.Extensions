...
public static class JustinsIEnumerableExtensions
{
    ...
    public static IEnumerable<T> WhereNotNull<T>(this IEnumerable<T?> source) where T : class => (source?.Where(item => item != null) ?? Empty<T>())!;
    ...
    public static string Join(this IEnumerable source, string separator) => string.Join(separator, source.OfType<object>());
}
#endif
